...
class Snake extends Component {
  ...
  magicMove(axis, axisValue) {
    ...
    if (axis === 'y') {
      return (axisValue < 0 ) ? (height - step) : ( axisValue + step > height ) ? 0 : axisValue
    } else {
      return (axisValue < 0 ) ? (width - step) : ( axisValue + step > width ) ? 0 : axisValue
    }
  }
  ...
  runHeadChange() {
    setInterval(() => this.setState({ snakeHeadindex: (this.state.snakeHeadindex +1) % snakeHead.length }),300)
  ...
  render () {
    return (
      <div className='snake'>
        <div className='snake-head' style={{top: this.state.y, left: this.state.x}}>{snakeHead[this.state.snakeHeadindex]}</div>
      </div>
    )
  }
